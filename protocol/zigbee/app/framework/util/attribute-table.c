...
{
  uint16_t attrMfgCode = 0xFFFF;
  uint16_t candidateMfgCode = 0xFFFF;
  EmberAfCluster *cluster = NULL;
  EmberAfAttributeMetadata *metadata = NULL;
  bool foundFirst = false;
  uint8_t foundAttrId = 0;
  uint16_t i;
  bool isMfgSpecCluster = (clusterId >= 0xFC00);
  ...
  for (i = 0;
       i < (isMfgSpecCluster ? clusterManufacturerCodeCount : 1);
       i++) {
    if (isMfgSpecCluster) {
