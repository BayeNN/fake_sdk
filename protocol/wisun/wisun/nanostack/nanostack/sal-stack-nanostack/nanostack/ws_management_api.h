...
#define REG_DOMAIN_APP  0xFE /**< application specific domain */
...
#define OPERATING_MODE_APP 0xFE /**< application specific mode */
...
/**
 * Set security protocol parameters
 *
 * Sets the parameters for security protocols configuration.
 *
 * When network size is changed using ws_management_network_size_set(),
 * it will override the configuration values set by this function.
 *
 * \param interface_id Network interface ID.
 * \param initial_key_imin Initial EAPOL-Key trickle Imin.
 * \param initial_key_imax Initial EAPOL-Key trickle Imax.
 * \param initial_key_min Minimum delay before initial EAPOL-Key.
 * \param initial_key_max Maximum delay before initial EAPOL-Key.
 *
 * \return 0, Init OK.
 * \return <0 Init fail.
 */
 ...
 * Get timing parameters
 *
 * Gets the parameters for security protocols configuration.
 *
 * \param interface_id Network interface ID.
 * \param initial_key_imin Initial EAPOL-Key trickle Imin.
 * \param initial_key_imax Initial EAPOL-Key trickle Imax.
...
/**
 * Set security timers configuration
 *
 * Sets the parameters for security timers configuration.
 *
 * \param interface_id Network interface ID.
 * \param revocat_lifetime_reduct Factor by which the active GTK lifetime is reduced
 *                                during node revocation procedures.
 * \param gtk_new_act_time The time at which the Border Router activates the next GTK
 *                         prior to expiration of the currently activated GTK.
 * \param gtk_new_install_req The point in the active GTK’s lifetime (as a percentage)
 *                            at which a new GTK must be installed on the Border Router.
 * \param gtk_max_mismatch Maximum time between a SUP detecting a GTKHASH mismatch and
 *                         the SUP initiating Msg1 of the authentication flow (causing
 *                         the BR to initiate a Group Key handshake with the SUP).
 *
 * \return 0, Init OK.
 * \return <0 Init fail.
 */
...
/**
 * Get security timers configuration
 *
 * Gets the parameters for security timers configuration.
 *
 * \param interface_id Network interface ID.
 * \param revocat_lifetime_reduct Factor by which the active GTK lifetime is reduced
 *                                during node revocation procedures.
