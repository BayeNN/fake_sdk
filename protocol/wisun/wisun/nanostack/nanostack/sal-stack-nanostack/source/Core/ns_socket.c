...
int16_t socket_buffer_sendmsg(int8_t sid, buffer_t *buf, const struct ns_msghdr *msg, int flags)
{
    ...
    {
        if (!buf) {
            // Check available space for DGRAM and RAW sockets.
            if (socket_ptr->sndq.data_byte_limit > 0 && socket_ptr->sndq.buf_overhead_byte_limit > 0) {
                int32_t space = sockbuf_space(&socket_ptr->sndq);
                if (space < payload_length) {
                    if (space < (int32_t) socket_ptr->sndq.low_water_mark) {
                        ret_val = NS_EWOULDBLOCK;
    ...
    /**
     * Allocate buffer on sockbuf if some space is reserved for DGRAM and RAW sockets.
     */
    if (socket_ptr->sndq.data_byte_limit > 0 && socket_ptr->sndq.buf_overhead_byte_limit > 0) {
        sockbuf_alloc(&socket_ptr->sndq, buf);
        sockbuf_allocated = 1;
    }
    ...
    switch (socket_ptr->type) {
        ...
        case SOCKET_TYPE_RAW:
            if (inet_pcb->protocol == IPV6_NH_ICMPV6) {
                // Remove ICMPv6 type, code and checksum from payload.
...
fail:
    ...
    if (buf) {
        if (sockbuf_allocated) {
            sockbuf_free(&buf->socket->sndq, buf);
        }
        buffer_free(buf);
    }
...
void socket_tx_buffer_event_and_free(buffer_t *buf, uint8_t status)
{
    ...
    if (buf) {
        if (buf->socket && buf->socket->type != SOCKET_TYPE_STREAM) {
            // Free memory allocated on sockbuf for DGRAM and RAW sockets.
            if (buf->socket->sndq.data_byte_limit > 0 && buf->socket->sndq.buf_overhead_byte_limit > 0) {
                // Buffer pointer was moved to insert IPv6 header in buffer overhead,
                // reset it to beginning of payload before calling sockbuf_free().
                buf->buf_ptr = buf->buf_end - buf->payload_length;
                sockbuf_free(&buf->socket->sndq, buf);
            }
        }
        buffer_free(buf);
