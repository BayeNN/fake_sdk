...
malformed:
    ...
    dodag = rpl_lookup_dodag(instance, dodagid);
    if (!dodag) {
        /* Policy can gate by DODAGID and other bits */
        if (!rpl_policy_join_dodag(domain, g_mop_prf, instance_id, dodagid)) {
            tr_info("Policy ignoring DODAG (%02x,%s)", g_mop_prf, trace_ipv6(dodagid));
    ...
    /* Reduce the scope of dodag_conf_ptr to fix a compilation warning when using IAR. */
    {
    ...
    if (!rpl_policy_join_config(domain, rpl_dodag_get_config(dodag), &become_leaf)) {
        goto invalid_parent;
    }

    /* Lookup or create DODAG Version state info */
    version = rpl_lookup_dodag_version(dodag, version_num);
    if (!version) {
    ...
    if (rpl_control_find_option_in_buffer(buf, 24, RPL_DAG_METRIC_OPTION, 0)) {
        become_leaf = true;
    }
