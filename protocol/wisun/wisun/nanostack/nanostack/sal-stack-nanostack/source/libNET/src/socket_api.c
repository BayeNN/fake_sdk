...
int8_t socket_bind(int8_t socket, const ns_address_t *address)
{
    ...
    if (identifier && identifier != inet_pcb->local_port) {
        // don't let them do so if a port was already chosen
        if (inet_pcb->local_port != 0) {
            return -4;
        }
        if (socket_port_validate(identifier, inet_pcb->protocol) == eOK) {
            inet_pcb->local_port = identifier;
        } else {
            return -2;
        }
    }
