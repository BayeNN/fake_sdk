...
static buffer_t *icmpv6_ns_handler(buffer_t *buf)
{
    ...
    na_buf = icmpv6_build_na(cur, true, !proxy, addr_is_ipv6_multicast(buf->dst_sa.address), target, aro_out.present ? &aro_out : NULL, buf->src_sa.address);
...
static buffer_t *icmpv6_redirect_handler(buffer_t *buf, protocol_interface_info_entry_t *cur)
{
    ...
    /* reduce the scope of tllao to fix a compilation warning when using IAR. */
    {
...
static buffer_t *icmpv6_na_handler(buffer_t *buf)
{
    ...
    /* Reduce the scope of aro to fix a compilation warning when using IAR. */
    {
