...
{
    ipv6_destination_t *dest_entry;
    protocol_interface_info_entry_t *outgoing_if;
    ...
    dest_entry = ipv6_destination_lookup_or_create(buf->dst_sa.address, cur ? cur->id : -1);
    if (!dest_entry) {
    ...
    outgoing_if = protocol_stack_interface_info_get_by_id(route->route_info.interface_id);
    if (!outgoing_if) {
...
buffer_t *ipv6_forwarding_up(buffer_t *buf)
{
    ...
    payload_length = common_read_16_bit(ptr);
    ptr += 2;
    ...
    for_us = intercept || ipv6_packet_is_for_us(buf);
