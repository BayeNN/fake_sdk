...
#warning "GAKs are saved in RAM for debug purposes"

static uint8_t saved_gak[GTK_NUM][GTK_LEN];
static void save_gak(uint8_t key_index, uint8_t *gak)
{
    memcpy(saved_gak[key_index], gak, GTK_LEN);
}

void ws_pae_backup_gak(uint8_t key_index, uint8_t *gak)
{
    memcpy(gak, saved_gak[key_index], GTK_LEN);
}
#endif
...
int8_t ws_pae_controller_configure(protocol_interface_info_entry_t *interface_ptr, struct ws_sec_timer_cfg_s *sec_timer_cfg, struct ws_sec_prot_cfg_s *sec_prot_cfg, struct ws_timing_cfg_s *timing_cfg)
{
    ...
    if (sec_prot_cfg) {
        ...
        controller->sec_cfg.prot_cfg.initial_key_min = sec_prot_cfg->initial_key_min;
        controller->sec_cfg.prot_cfg.initial_key_max = sec_prot_cfg->initial_key_max;
    }
...
#endif
    // can't be reach if compiled without HAVE_PAE_AUTH
    return 0;
#if defined __ICCARM__
