...
#if SLI_WISUN_DISABLE_FRAME_SECURITY
    // In normal operation, a neighbor becomes trusted when the node receives
    // an encrypted frame from it. When frame security is disabled, the node will
    // trust all neighbors by default.
    mac_neighbor_table_trusted_neighbor(mac_neighbor_info(interface), neighbor, true);
#endif
...
static void ws_bootstrap_pan_config_analyse(struct protocol_interface_info_entry *cur, const struct mcps_data_ind_s *data, const struct mcps_data_ie_list *ie_ext, ws_utt_ie_t *ws_utt, ws_us_ie_t *ws_us)
{
     ...
     * a PANVER-IE / PAN Version greater than or equal to the receiving nodeâ€™s current PAN version.
     *
     * A inconsistent transmission is defined as:
     *
     *  A PAN Configuration with PAN-ID matching that of the receiving node and a
     *  PANVER-IE / PAN Version that is less than the receiving nodeâ€™s current PAN version.
     */
...
int ws_bootstrap_init(int8_t interface_id, net_6lowpan_mode_e bootstrap_mode)
{
    ...
    mpx_api = ws_llc_mpx_api_get(cur);
    if (!mpx_api) {
        ret_val =  -4;
        goto init_fail;
    }
...
static int ws_bootstrap_set_domain_rf_config(protocol_interface_info_entry_t *cur)
{
    ...
    rf_configs.channel_0_center_frequency = (uint32_t)cur->ws_info->hopping_schdule.ch0_freq * 1000;
...
static void ws_bootstrap_state_change(protocol_interface_info_entry_t *cur, icmp_state_t nwk_bootstrap_state)
{
    ...
    // Post BOOTSTRAP_STATE_CHANGED event to application
    ws_nwk_event_post(cur, ARM_NWK_BOOTSTRAP_STATE_CHANGED);
}
...
void ws_bootstrap_state_machine(protocol_interface_info_entry_t *cur)
{
    ...
    switch (cur->nwk_bootstrap_state) {
        ...
        case ER_PANA_AUTH:
            ...
            new_default = cur->ws_info->weakest_received_rssi - 1;
