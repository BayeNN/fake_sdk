...
{
    reassembly_entry_t *frag_ptr;
    uint16_t fragment_last;
    uint16_t hole_off;
    uint16_t *prev_ptr;
    ...
    frag_ptr = reassembly_already_action(&interface_ptr->rx_list, buf, datagram_tag, datagram_size);
    ...
    fragment_last = fragment_first + ipv6_size - 1;
     ...
     */
    hole_off = frag_ptr->offset;
    prev_ptr = &frag_ptr->offset;
    do {
