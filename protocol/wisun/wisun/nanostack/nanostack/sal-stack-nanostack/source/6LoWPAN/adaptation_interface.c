...
int8_t lowpan_adaptation_interface_tx(protocol_interface_info_entry_t *cur, buffer_t *buf)
{
    ...
    //Check packet size
    fragmented_needed = lowpan_adaptation_request_longer_than_mtu(cur, buf, interface_ptr);
    ...
    indirect = buf->link_specific.ieee802_15_4.indirectTxProcess;
    ...
    tx_ptr = lowpan_adaptation_tx_process_init(interface_ptr, indirect, fragmented_needed, is_unicast);
    if (!tx_ptr) {
        goto tx_error_handler;
    }
