...
int ws_management_sec_prot_parameters_set(
    int8_t interface_id,
    uint16_t initial_key_imin,
    uint16_t initial_key_imax,
    uint16_t initial_key_min,
    uint16_t initial_key_max)
{
    protocol_interface_info_entry_t *cur;
    ...
    ws_sec_prot_cfg_t cfg;
    if (ws_cfg_sec_prot_get(&cfg, NULL) < 0) {
        return -2;
    }

    ws_sec_prot_cfg_t cfg_default;
    if (ws_cfg_sec_prot_default_set(&cfg_default) < 0) {
        return -2;
    }

    if (initial_key_imin > 0) {
        cfg.initial_key_imin = initial_key_imin;
    } else {
        cfg.initial_key_imin = cfg_default.initial_key_imin;
    }

    if (initial_key_imax > 0) {
        cfg.initial_key_imax = initial_key_imax;
    } else {
        cfg.initial_key_imax = cfg_default.initial_key_imax;
    }

    if (initial_key_min > 0) {
        cfg.initial_key_min = initial_key_min;
    } else {
        cfg.initial_key_min = cfg_default.initial_key_min;
    }

    if (initial_key_max > 0) {
        cfg.initial_key_max = initial_key_max;
    } else {
        cfg.initial_key_max = cfg_default.initial_key_max;
    }

    if (ws_cfg_sec_prot_set(cur, NULL, &cfg, 0) < 0) {
        return -3;
    }
...
int ws_management_sec_prot_parameters_get(
    int8_t interface_id,
    uint16_t *initial_key_imin,
    uint16_t *initial_key_imax,
    uint16_t *initial_key_min,
    uint16_t *initial_key_max)
{
    ...
    if (!initial_key_imin || !initial_key_imax || !initial_key_min || !initial_key_max) {
        return -2;
    }
    ...
    *initial_key_imin = cfg.initial_key_imin;
    *initial_key_imax = cfg.initial_key_imax;
    *initial_key_min = cfg.initial_key_min;
    *initial_key_max = cfg.initial_key_max;

    return 0;
}

int ws_management_sec_timer_parameters_set(
    int8_t interface_id,
    uint16_t revocat_lifetime_reduct,
    uint16_t gtk_new_act_time,
    uint8_t gtk_new_install_req,
    uint16_t gtk_max_mismatch)
{
    ...
    ws_sec_timer_cfg_t cfg_default;
    if (ws_cfg_sec_timer_default_set(&cfg_default) < 0) {
        return -2;
    }
    ...
    if (revocat_lifetime_reduct > 0) {
        ...
        cfg.revocat_lifetime_reduct = cfg_default.revocat_lifetime_reduct;
    }

    if (gtk_new_act_time > 0) {
        cfg.gtk_new_act_time = gtk_new_act_time;
    } else {
        cfg.gtk_new_act_time = cfg_default.gtk_new_act_time;
    }

    if (gtk_new_install_req > 0) {
        cfg.gtk_new_install_req = gtk_new_install_req;
    } else {
        cfg.gtk_new_install_req = cfg_default.gtk_new_install_req;
    }

    if (gtk_max_mismatch > 0) {
        cfg.gtk_max_mismatch = gtk_max_mismatch;
    } else {
        cfg.gtk_max_mismatch = cfg_default.gtk_max_mismatch;
    }
...
int ws_management_sec_timer_parameters_get(
    int8_t interface_id,
    uint16_t *revocat_lifetime_reduct,
    uint16_t *gtk_new_act_time,
    uint8_t *gtk_new_install_req,
    uint16_t *gtk_max_mismatch)
{
    ...
    if (!revocat_lifetime_reduct || !gtk_new_act_time || !gtk_new_install_req || !gtk_max_mismatch) {
        return -2;
    }
    ...
    *revocat_lifetime_reduct = cfg.revocat_lifetime_reduct;
    *gtk_new_act_time = cfg.gtk_new_act_time;
    *gtk_new_install_req = cfg.gtk_new_install_req;
    *gtk_max_mismatch = cfg.gtk_max_mismatch;
