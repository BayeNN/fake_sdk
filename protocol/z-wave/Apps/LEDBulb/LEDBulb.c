...
sl_simple_rgbw_pwm_led_context_t simple_rgbw_pwm_led_context = {
  .port[SL_SIMPLE_RGBW_PWM_LED_COLOR_R] = gpioPortD,
  .pin[SL_SIMPLE_RGBW_PWM_LED_COLOR_R] = 10,
  ...
  .port[SL_SIMPLE_RGBW_PWM_LED_COLOR_G] = gpioPortD,
  .pin[SL_SIMPLE_RGBW_PWM_LED_COLOR_G] = 11,
  ...
  .port[SL_SIMPLE_RGBW_PWM_LED_COLOR_B] = gpioPortD,
  .pin[SL_SIMPLE_RGBW_PWM_LED_COLOR_B] = 12,
  ...
  .port[SL_SIMPLE_RGBW_PWM_LED_COLOR_W] = gpioPortF,
  .pin[SL_SIMPLE_RGBW_PWM_LED_COLOR_W] = 4,
  ...
  .location[SL_SIMPLE_RGBW_PWM_LED_COLOR_W] = 25,
  .timer = TIMER1,
  .frequency = 10000,
  .resolution = 256
};
const sl_led_rgbw_pwm_t sl_led = {
  ...
  uint8_t cmdLength)
{
  ...
  switch (pCmd->ZW_Common.cmdClass)
  {
    ...
    case COMMAND_CLASS_SWITCH_COLOR_V3:
      frame_status = CC_ColorSwitch_handler(rxOpt, pCmd, cmdLength);
      break;

    case COMMAND_CLASS_SWITCH_MULTILEVEL_V4:
...
{
  /* Basic Set Command is mapped to Multilevel Switch Set */
  return cc_multilevel_switch_set_handler(val, 0, endpoint);
}
...
void callback_RED(s_colorComponent * colorComponent)
{
  ...
  DPRINTF("%s Setting RGBW=(%u,%u,%u,%u)\n", __func__,
          ...
          (multilevel_switch_value * color_switch_max) / multilevel_switch_max);
  sl_led_set_rgbw_color(&sl_led,
                        (color_switch_red_value * multilevel_switch_value) / multilevel_switch_max,
                        (color_switch_green_value * multilevel_switch_value) / multilevel_switch_max,
                        (color_switch_blue_value * multilevel_switch_value) / multilevel_switch_max,
                        (multilevel_switch_value * color_switch_max) / multilevel_switch_max);
