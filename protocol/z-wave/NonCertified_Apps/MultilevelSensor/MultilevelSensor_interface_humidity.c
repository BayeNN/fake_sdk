...
bool MultilevelSensor_interface_humidity_read(sl_sensor_read_result_t* o_result, uint8_t i_scale)
{
  ...
  if(o_result != NULL)
  {
    ...
    if(true == Sensor_si7021_read_humidity(&humidity))
    {
      retval = true;
      ...
      o_result->raw_result[3] = (humidity   &0xFF);
      o_result->raw_result[2] = (humidity>>8 )&0xFF;
      o_result->raw_result[1] = (humidity>>16)&0xFF;
      o_result->raw_result[0] = (humidity>>24)&0xFF;
    }
  }
  return retval;
}
