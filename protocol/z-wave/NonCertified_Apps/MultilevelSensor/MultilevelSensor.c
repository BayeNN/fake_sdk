...
#define BATTERY_REPORT_BTN   APP_BUTTON_A         // This button cannot wake up the device from EM4
                                                  // (i.e. it will generally not work with Sensor)
...
static uint8_t supportedHeatNotificationEvents[] = {NOTIFICATION_EVENT_HEAT_ALARM_RAPID_TEMPERATURE_RISE,
...
//Periodic timer for heat notification checking
static SSwTimer HeatNotificationPeriodicTimer;
...
static void
PeriodicHeatEventCheck(SSwTimer *pTimer)
{
  UNUSED(pTimer);

  sl_sensor_read_result_t read_result;

  if( true == MultilevelSensor_interface_temperature_read(&read_result, SL_SENSOR_SCALE_CELSIUS))
  {
    cc_config_parameter_buffer_t minimum_celsius_limit;
    cc_config_parameter_buffer_t maximum_celsius_limit;
    int32_t raw_celsius_temperature;

    sl_cc_configuration_get(sl_configuration_index_minimum_celsius_temperature_limit, &minimum_celsius_limit);
    sl_cc_configuration_get(sl_configuration_index_maximum_celsius_temperature_limit, &maximum_celsius_limit);

    raw_celsius_temperature  =  read_result.raw_result[3];
    raw_celsius_temperature |= (read_result.raw_result[2]<<8);
    raw_celsius_temperature |= (read_result.raw_result[1]<<16);
    raw_celsius_temperature |= (read_result.raw_result[0]<<24);
    raw_celsius_temperature /= 1000;

    uint8_t notificationEvent = (uint8_t)NOTIFICATION_EVENT_HEAT_ALARM_UNKNOWN_EVENT;

    if(raw_celsius_temperature < minimum_celsius_limit.data_buffer.as_int32)
    {
    	notificationEvent = NOTIFICATION_EVENT_HEAT_ALARM_RAPID_TEMPERATURE_FALL;
    }
    else if(raw_celsius_temperature > maximum_celsius_limit.data_buffer.as_int32)
    {
        notificationEvent = NOTIFICATION_EVENT_HEAT_ALARM_RAPID_TEMPERATURE_RISE;
    }

    if(notificationEvent != NOTIFICATION_EVENT_HEAT_ALARM_UNKNOWN_EVENT)
    {
...
{
  DPRINT("\r\nMultilevel Sensor Main App/Task started! \n");
  ...
  AppTimerRegister(&HeatNotificationPeriodicTimer, true, PeriodicHeatEventCheck);
  TimerStart(&HeatNotificationPeriodicTimer, HEAT_EVENT_CHECK_PERIOD_MS);
  ...
  DPRINT("\r\nMultilevel Sensor Event processor Started\n");
...
static void doRemainingInitialization()
{
  ...
  AddNotification(
      &lifelineProfile,
      NOTIFICATION_TYPE_HEAT_ALARM,
      supportedHeatNotificationEvents,
      sizeof(supportedHeatNotificationEvents)/sizeof(supportedHeatNotificationEvents[0]),
      false,
      0,
      NOTIFICATION_STATUS_UNSOLICIT_ACTIVATED,
      filesExist
  ...
  uint8_t cmdLength)
{
  ...
  switch (pCmd->ZW_Common.cmdClass)
  {
    ...
    case COMMAND_CLASS_CONFIGURATION_V4:
      frame_status = CC_Configuration_handler(rxOpt, pCmd, cmdLength);
      break;
      
    case COMMAND_CLASS_SENSOR_MULTILEVEL:
      frame_status = sl_cc_multilevel_sensor_handler(rxOpt, pCmd, cmdLength);
      break;
  }
  return frame_status;
}
