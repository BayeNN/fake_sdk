  ...
  uint8_t ep)
{
  if ((LIFELINE_GROUP_ID == groupIden) && (LIFELINE_ENDPOINT_ALLOWED != ep))
  {
    ...
    // for group 1 (LIFELINE) if the endpoint is other than  than LIFELINE_ENDPOINT_ALLOWED.
    return 0;
  }
...
{
  ASSERT(pNodeToAdd->nodeId <= ZW_MAX_NODES); // This makes this function LR aware and avoids it in tests!
  /*
  ...
  if (ZAF_GetInclusionMode() == EINCLUSIONMODE_ZWAVE_LR)
  {
    ...
    // Only accept the LIFELINE group ID! (LR nodes are not associating with each other)
    ...
    DPRINTF("Detected InclusionMode: EINCLUSIONMODE_ZWAVE_LR \n");
  }
   ...
   */
  indx++;
  pCurrentNode = GetNode(endpoint, groupID, indx);  // This returns an empty node.
...
NVM_Action(NVM_ACTION action)
{
  ...
  switch(action)
  {
    ...
    default:
      DPRINTF("FATAL: Case not handled. Invalid input.");
...
/**
 * @brief Searches through the group mapping table and finds a mapping between groupID and endpoint and returns the mapping through the same pointer.
 *
...
AssGroupMappingLookUp(
    ...
    uint8_t* pGroupID)
{
  if ((ENDPOINT_ROOT == *pEndpoint) && (LIFELINE_GROUP_ID != *pGroupID) && NON_NULL(pGroupMappingTable))
  {
    /* Use existing Group mapping to get endpoint groupId. */
    uint8_t i;
