...
typedef struct _cc_config_parameter_attributes_t{
  ...
  const cc_config_parameter_size_t  size;          ///< Size of the parameter in bytes
...
typedef struct {
  ...
  const uint16_t next_number;        ///< Next parameter number
  const nvm3_ObjectKey_t file_id;    ///< ID of the configuration file
  const cc_config_parameter_attributes_t attributes; ///< Attributes of the parameters
...
*/
received_frame_status_t CC_Configuration_handler( RECEIVE_OPTIONS_TYPE_EX *rxOpt,
