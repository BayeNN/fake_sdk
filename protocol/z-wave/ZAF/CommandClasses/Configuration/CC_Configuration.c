                                  ...
                                  const cc_config_parameter_value_t* pField)
{
      ...
      (pField != NULL))
  {
    ...
    if(NULL != parameter_buffer->metadata)
    {
      ...
      while((src_ix >= 0) && (dest_ix < sizeof(pFrame->as_uint8_array)))
      {
                                    ...
                                    const uint8_t cmdLength)
{
  ...
  uint16_t parameter_number = ((uint16_t)(pCfgNameGetFrame->parameterNumber1<<8)) | ((uint16_t)(pCfgNameGetFrame->parameterNumber2));
  ...
  uint8_t info1_offset_in_struct = offsetof(ZW_CONFIGURATION_INFO_REPORT_4BYTE_V4_FRAME , info1);
  ...
  if(is_io_transaction_success != false)
  {
    ...
    {
      uint16_t remaining_byte_count = (uint16_t)info_str_length - str_pointer;
      uint16_t byte_count_to_send   = remaining_byte_count>=payload_limit?payload_limit:remaining_byte_count;
      uint8_t frame_size_bytes      = 5 /*header size*/ + (uint8_t)byte_count_to_send;
                                      ...
                                      const uint8_t cmdLength)
{
  ...
  if(is_io_transaction_success != false)
  {
    ...
    uint16_t str_pointer = 0;
    uint8_t name1_offset_in_struct = offsetof(ZW_CONFIGURATION_NAME_REPORT_4BYTE_V4_FRAME , name1);
    ...
    {
      uint16_t remaining_byte_count = (uint16_t)name_str_length - str_pointer;
      uint16_t byte_count_to_send = remaining_byte_count>=payload_limit?payload_limit:remaining_byte_count;
      uint8_t  frame_size_bytes = 5 /*header size*/ + (uint8_t)byte_count_to_send;
                                             ...
                                             const uint8_t cmdLength)
{
  ...
  if((is_io_transaction_success == true) && (parameter_buffer.metadata->number <= parameter_number))
  {
    is_io_transaction_success = sl_cc_configuration_get(parameter_number, &parameter_buffer);
  }
  ...
  if(is_io_transaction_success != false)
  {
    ...
    pTxBuf->ZW_ConfigurationPropertiesReport4byteV4Frame.nextParameterNumber1 = (parameter_buffer.metadata->next_number >> 8)&0xFF;
    pTxBuf->ZW_ConfigurationPropertiesReport4byteV4Frame.nextParameterNumber2 = (parameter_buffer.metadata->next_number)&0xFF;
                                      ...
                                      const uint8_t cmdLength)
{
  ...
  uint16_t parameter_offset    = (uint16_t)(pCfgBulkGetFrame->parameterOffset1<<8) |
                                 (uint16_t)(pCfgBulkGetFrame->parameterOffset2);
                                      ...
                                      const uint8_t cmdLength)
{
  ...
  uint16_t parameter_offset = (uint16_t)(p_raw_cmd[cmd_index]<<8); /*Parameter Offset 1*/
  cmd_index++;
  parameter_offset |= (uint16_t)p_raw_cmd[cmd_index];              /*Parameter Offset 2*/
  cmd_index++;
                                              ...
                                              const bool flag_handshake)
{
  ...
  if(status == true)
  {
    ...
    pTxBuf->ZW_ConfigurationBulkReport4byteV4Frame.parameterOffset1   = (parameter_offset >> 8)&0xFF;
    pTxBuf->ZW_ConfigurationBulkReport4byteV4Frame.parameterOffset2   =  parameter_offset & 0xFF;
    ...
    while(status == true)
    {
                      ...
                      sizeof(cc_config_parameter_value_t)))
      {
        ...
        pTxBuf->ZW_ConfigurationBulkReport4byteV4Frame.properties1 &= ~(1<<7);
      }
      ...
      {
        uint8_t frame_size_bytes = 7 /*header size*/ + current_payload_size;
...
sl_cc_configuration_calc_reports_to_follow(size_t data_length, size_t payload_limit)
{
  ...
  reports_to_follow_count += (uint16_t)(((data_length % payload_limit) != 0)?1:0); /*Round up*/
