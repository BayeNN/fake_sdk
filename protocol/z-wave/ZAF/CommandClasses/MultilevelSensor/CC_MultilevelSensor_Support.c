...
static void 
sl_cc_multilevel_sensor_operation_report_stx(TRANSMIT_OPTIONS_TYPE_SINGLE_EX txOptions, const void* pData);
                                ...
                                uint8_t cmdLength)
{
  ...
  number_of_registered_sensors = sl_cc_multilevel_sensor_get_number_of_registered_sensors();
  
  if(number_of_registered_sensors > 0)
  {
  ...
  {
    return_value = RECEIVED_FRAME_STATUS_FAIL;
  }
...
void sl_cc_multilevel_sensor_init(void)
{
  ...
  AppTimerRegister(&cc_multilevel_sensor_autoreport_timer, true, sl_cc_multilevel_sensor_autoreport_callback);
  TimerStart(&cc_multilevel_sensor_autoreport_timer, SL_MULTILEVEL_SENSOR_DEFAULT_AUTOREPORT_PEDIOD_MS);
}
...
static void sl_cc_multilevel_sensor_autoreport_callback(SSwTimer *pTimer)
{
  ...
  */
  ...
  {
    ZAF_TSE_Trigger((void*)&sl_cc_multilevel_sensor_operation_report_stx, &zaf_tse_local_actuation, true);
  }
}
 ...
 */
static void sl_cc_multilevel_sensor_operation_report_stx(TRANSMIT_OPTIONS_TYPE_SINGLE_EX txOptions, const void* pData)
{
  UNUSED(pData);
                                                                    ...
                                                                    uint8_t cmdLength)
{
  ...
  uint8_t payload_length           = 11; /*payload*/
...
REGISTER_CC(COMMAND_CLASS_SENSOR_MULTILEVEL_V11, SENSOR_MULTILEVEL_VERSION_V11, sl_cc_multilevel_sensor_handler);
// -----------------------------------------------------------------------------
