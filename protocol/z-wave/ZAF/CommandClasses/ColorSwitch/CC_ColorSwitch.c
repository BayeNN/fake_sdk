                       ...
                       uint8_t cmdLength)
{
  switch (pCmd->ZW_Common.cmd)
  {
    case SWITCH_COLOR_SUPPORTED_GET:
    {
      ...
      pTxBuf->ZW_SwitchColorSupportedReportV3Frame.colorComponentMask1 = colorCompMask & 0xFF;
      pTxBuf->ZW_SwitchColorSupportedReportV3Frame.colorComponentMask2 = colorCompMask >> 8;
    ...
    case SWITCH_COLOR_SET:
    {
      ...
      for (int i = 0; i < colorCount; i++)
      {
        ...
        color->rxOpt = *pRxOpt;
        colorsChangingBitmask |= (1 << color->colorId);
    ...
    case SWITCH_COLOR_STOP_LEVEL_CHANGE:
    {
      ...
      {
        colorsChangingBitmask &= ~(1 << color->colorId);
        if (false == ZAF_TSE_Trigger((void *)CC_ColorSwitch_report_stx,
                                     color,
                                     false))
        {
...
void CC_ColorSwitch_ColorChanged_cb(s_Actuator *pObj)
{
  ...
  if(ZAF_Actuator_GetTargetValue(&color->obj) == ZAF_Actuator_GetCurrentValue(&color->obj))
  {
    ...
    if (0 == colorsChangingBitmask)
    {
      if(color->rxOpt.bSupervisionActive && color->rxOpt.statusUpdate)
      {
        // Send Supervision Report Success.
        TRANSMIT_OPTIONS_TYPE_SINGLE_EX *pTxOptionsEx;
        RxToTxOptions(&color->rxOpt, &pTxOptionsEx);
        CmdClassSupervisionReportSend(pTxOptionsEx,
...
void CC_ColorSwitch_report_stx(TRANSMIT_OPTIONS_TYPE_SINGLE_EX txOptions, s_colorComponent *pData)
{
  DPRINTF("* %s() *\n"
      "\ttxOpt.src = %d\n"
      "\ttxOpt.options %#02x\n"
...
REGISTER_CC(COMMAND_CLASS_SWITCH_COLOR, SWITCH_COLOR_VERSION_V3, CC_ColorSwitch_handler);
